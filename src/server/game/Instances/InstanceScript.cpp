...
void InstanceScript::GetScenarioByID(Player* p_Player, bool p_Isarea)
{
    TeamId tid;
    uint32 zoneid;
    tid = TEAM_ALLIANCE;
    ...
    if (p_Player)
    {
        tid = p_Player->GetTeamId();
        zoneid = p_Isarea ? p_Player->GetAreaId() : p_Player->GetZoneId();
    }
    InstanceMap* map = instance->ToInstanceMap();

    if (InstanceScenario* instanceScenario = sScenarioMgr->CreateInstanceScenario(map, tid))
    {
   ...
   else
   {
       if (keyOwner = ObjectAccessor::FindPlayer(m_ownerGuid))
       {
           if (_item = keyOwner->GetItemByGuid(m_itemGuid))
               ...
               QueryResult result = CharacterDatabase.PQuery("UPDATE challenge_key SET KeyIsCharded = 0, InstanceID = 0 WHERE guid = %u", m_ownerGuid.ToString());
       }
       else
           QueryResult result = CharacterDatabase.PQuery("UPDATE challenge_key SET KeyIsCharded = 0, InstanceID = 0 WHERE guid = %u", m_ownerGuid.ToString());
   }
    ...
    challengeData->ID = sObjectMgr->ScenarioGuidId();
    ...
    if (InstanceScript* script = sChallengeModeMgr->GetInstanceScript())
    ...
    _map->ApplyOnEveryPlayer([&](Player* player)
    {
        for (uint32 i = 0; i < MAX_SPECIALIZATIONS; ++i)
        {
            if (ChrSpecializationEntry const* specialization = sDB2Manager.GetChrSpecializationByIndex(0, i)) {
                ...
                member.specId = specialization->ID;
                ...
                sChallengeModeMgr->SendChallengeModeMapStatsUpdate(player);

                player->UpdateCriteria(CRITERIA_TYPE_COMPLETE_CHALLENGE_MODE, _mapID, _rewardLevel);

                player->RemoveAura(206151);
                player->CastSpell(player, 305284, true);
                player->KilledMonsterCredit(542180); // for daily event quest
            }
        }
    });

    if (sChallengeModeMgr->GetChallengeTimer() < 9 * MINUTE)
    {
