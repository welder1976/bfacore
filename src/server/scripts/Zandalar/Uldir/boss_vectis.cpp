...
class spell_omega_vector : public AuraScript
{
    ...
    void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        if (GetEffect(EFFECT_0)->GetTickNumber() >= uint32(GetEffect(EFFECT_0)->GetTotalTicks()))//this prevent a loop in the server when the aura is cleanned before expire
            if (Unit* target = GetTarget())
            {
                Player* nextPlayer = target->SelectNearestPlayer(500.0f);
                if (nextPlayer)
                    target->CastSpell(nextPlayer, GetSpellInfo()->Id);
                if (Unit* boss = target->FindNearestCreature(ULDIR_BOSS_VECTIS, 500.0f))
                {
...
class spell_blood_geyser_damage : public SpellScript
{
    ...
    void DoEffectHitTarget(SpellEffIndex /*effIndex*/)
    {
        if (Unit* hitUnit = GetHitUnit())
        {
            if (Creature* vectis = hitUnit->FindNearestCreature(ULDIR_BOSS_VECTIS, 500.0f))
...
{
    Position movePos;
    float ori = M_PI_2 + M_PI + frand(0.f, float(M_PI));
...
struct areatrigger_blood_geyser : AreaTriggerAI
{
    ...
    {
        if (unit)
            if (Creature* vectis = unit->FindNearestCreature(ULDIR_BOSS_VECTIS, 500.0f))
...
struct areatrigger_liquefy : AreaTriggerAI
{
    ...
    void OnUnitEnter(Unit* unit)
    {
        if (unit)
        {
            if (Creature* vectis = unit->FindNearestCreature(ULDIR_BOSS_VECTIS, 100.0f))
...
class spell_burning_lesions : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex effIndex)
    {
        if (Unit* caster = GetCaster())
        {
            ...
            {
                if (Creature* vectis = itr->FindNearestCreature(ULDIR_BOSS_VECTIS, 500.0f))
